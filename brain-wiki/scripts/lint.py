...
def scan_contradictions(memory_text: str) -> dict[str, str]:
    ...
    for topic_dir in cfg.wiki_dir.iterdir():
        ...
        report = call_local(prompt, CONTRADICTION_SYSTEM, timeout=180)
...
"""
    for issue in issues:
        ...
        prompt = f"Topic: {topic_name}\n\nExisting pages:\n\n{pages_block}"
        content = call_local(prompt, OVERVIEW_INIT_SYSTEM, timeout=180)
            ...
            for issue in missing_xrefs:
                ...
                if m:
                    ...
                    if tgt.exists():
                        ...
                        backpatch_file(src, entry, call_local)
    else:
